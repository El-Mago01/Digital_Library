...
def return_to_home(current_function, outcome: dict, sorting_command: dict):
    """
    This helper function returns all the books in the DB to the homepage based upon the
    configuration of the sorting command. IT will further support the display of the outcome
    of the service which is described in "current function"
    :param current_function:
    :param outcome: a dict with result code, message to be returned to the front end page
    :param sorting_command: sort_by title or author, listed based upon direction (asc, desc)
    :return: input for rendering the template
    """
...
def return_to_authors_list(current_function, outcome: dict):
    """
    This helper function returns all the Authors in the DB to the list_authors.html
    based upon the  configuration of the sorting command. It will further support
    the display of the outcome of the service which is described in "current function"
    :param current_function:
    :param outcome: a dict with result code, message to be returned to the front end page
    :param sorting_command: listing based upon direction (asc, desc)
    :return: input for rendering the template
    """
    all_authors = ds.get_all_authors_from_db()
    for author_id, name, olid_author, cover_img, birth_year, death_year in all_authors:
        print(
            f"author_id: {author_id}, name: {name}, olid_author: {olid_author}, cover_image: {cover_img}, birth_year: {birth_year}, death_year: {death_year}"
        )
    return render_template(
        "list_authors.html",
        app_name=app_name,
...
def home():
    """
    Show all the books in the library. based upon the input parameters
    :param sorting_command
    :return: The current books in the library
    """
    outcome = {"result": 200, "message": "No messages"}
    sorting_command = {
        "sort_by": request.args.get("sort_by", "books"),
        "direction": request.args.get("direction", "asc"),
    }

    return return_to_home(
        "Current books in MAGO Library",
        outcome,
        sorting_command)
...
def add_book():
    ...
    The call is made to open library to search for this title/author. A list of dicts with all
    relevant books
    is returned. The title/author/publishing_year are displayed in a drop-down list with the
    olid_book_id as value that
    will be returned for the selected book.  The user has now 2 options, either it selects a
    book from the list and press "add book" or clicks "Manually add books".
    ...
    print(
        f"book_title  {book_title} saved_book_title, {saved_book_title}, saved_author_name, {saved_author_name}"
    )
    ...
    if len(saved_book_title) == 0 and len(book_title) != 0:
        ...
        try:
            ...
            outcome = {
                "result": 200,
                "message": f"Could not fetch book/author "
                f"information due to an exception: \n{e}. \nTry again later.",
            }
            sorting_command = {"sort_by": "books", "direction": "asc"}
            return return_to_home(
                "Adding a new author",
                outcome,
                sorting_command)
        # possible_books contains a list of tuples with a Book class and the
        # name of the author
        book_not_found = True
    ...
    # STEP 3: 2nd POST request to add the selected book (and indirectly the author if needed)
    # Here there are 3 scenarios for this stage:
    ...
    # 3. The user selected the manual storage option. In that case, the rout
    # /manually_add_book is selected
    ...
    if len(book_title) != 0 or len(author_name) != 0:
        ...
        try:
            ...
            outcome = {
                "result": 200,
                "message": f"Could not fetch book/author information due to an exception: \n{e}. \nTry again later.",
            }
            sorting_command = {"sort_by": "books", "direction": "asc"}
            return return_to_home(
                "Adding a new author",
    ...
    if (
        sim_stored_book != ""
    ):  # This is case 3, the user selected one of the already stored books
    ...
    try:
        ...
        abort(
            500,
            description=f"Unexpected returned input provided by the add_book.html form . {e}",
        )
    ...
    for the_book in possible_books:
        try:
            if (
                the_book["olid_book_id"] == selected_olid_book
                and selected_olid_book != ""
            ):
                book_dict_to_store = the_book
                author_olid = the_book.get("olid_author_id")[0]
                author_id_and_name = ds.check_author_already_in_db(
                    the_book["title"], author_olid
                )
                if len(author_id_and_name) == 0:
                    ...
                    if new_author is not None:
                        ...
                        outcome_message += (
                            f"<p>Successfully added author {
                                new_author.name}.</p>")
                else:
                    author_id, author_name = author_id_and_name[
                        0
                    ]  # If the author was already stored in the DB.
                break

                # Note, if author is already stored, nothing needs to be done.
                # Only the author_id is returned.
        except KeyError as e:
            message = f"Internal Error: could not find the olid_author or olid_book. Details: {e}"
    ...
    outcome = {
        "result": 500,
        "message": "An internal error occurred. Could not store book",
    }
    sorting_command = {"sort_by": "books", "direction": "asc"}
    if book_dict_to_store != {}:
        if author_id == -1:
            outcome = (
                "Could not find a related book. Please ensure to store an "
        ...
        db.session.commit()
        outcome_message += f"<p>Successfully added book {
            book_to_store.title}</p>"
...
def manually_add_book():
    """
    Enables the manually adding of a new book. Important, the author has to be stored on forehand. The user gets
    a list of authors already stored in the DB. IF the right author is not in there, there is a button to
    store the author first.
    :return:
    """
    # 1st REQUEST, a GET request
    if request.method == "GET":
        ...
        logging.info(
            f"Manually adding a book with GET request for title: {book_title}")

        return render_template(
            "manually_add_book.html",
    ...
    try:
        ...
        abort(
            404,
            description=f"Please check your input. \nWrong values received "
            f"for either/both author or publication year. \n{e}.",
        )
    if author_id == -1:
        abort(404, description="No author_id received. Please select an author")
    ...
    outcome = {
        "result": 200,
        "message": f"Successfully added book {received_book.title}",
    }
    sorting_command = {"sort_by": "books", "direction": "asc"}
...
def update_book():
    """
    Update a book in the database based on book_id parameter. The update feature as such is
    using 2 end points (routes). This is the initial reception of the request to update a
    specific book identified by the input parameter book_id.

    The user will get a html file back where the book elements are already pre-filled.
    end_point '/updated_book will take care that the changes are stored.
    :param book_id
    :return:
    """
    try:
...
def updated_book():
    """
    The user returned the form with the changes to the book elements. This endpoint takes care
    that these changes are correctly stored in the database.
    :return:
    """
    try:
        book_id = int(request.form["book_id"])
        updated_publication_year = int(
            request.form.get(
                "book_publication_year", "0"))
        updated_author_id = request.form.get("author_id", "-1")
    except ValueError as e:
    ...
    if len(updated_cover_image) != 0:
        book_to_update.cover_img = df.compile_img_url(
            updated_cover_image, True)
    if updated_author_id != "Open to select author" and updated_author_id != "-1":
        book_to_update.author_id = updated_author_id

    db.session.commit()
    outcome = {
        "result": 200,
        "message": f"Update of book: id-{book_id} successful"}
    sorting_command = {"sort_by": "books", "direction": "asc"}
...
def delete_book():
    """
    Delete a book from the DB based upon input parameter "book_id"
    :param book_id, identifying the book to be deleted
    :return:
    """
    try:
        received_book_id = int(request.args.get("book_id", "-1"))
    ...
    outcome = {
        "result": 200,
        "message": f"Deletion of book: id-{received_book_id} successful",
    }
    sorting_command = {"sort_by": "books", "direction": "asc"}
...
def list_authors():
    """
    List all the available authors in the DB on a new list_authors.html.
    :return:
    """
    direction = request.args.get("direction", "asc")
...
def add_author():
    ...
    # 1. The user selected 1 of the possible authors. In that case, store the selected
    # author fields in the DB
    # 2. The user entered a new author_name. In that case, it should be handled as if
    # the user entered in this stage
    ...
    if (
        sim_stored_author != ""
    ):  # This is case 3, the user selected one of the already stored authors
        return redirect(url_for("home"))
    ...
    try:
        ...
        # a list, the list of possible authors from the 1st POST stage, is
        # stored as JSON in this jinja2 variable
        ...
        print(
            "This is the final selected author",
            selected_author,
            possible_authors[selected_author]["name"],
        )
    except ValueError as e:
        abort(
            500,
            description=f"return input provided by the add_author.html "
            f"form is not an integer. See message: {e}",
        )
    ...
    outcome = {
        "result": 200,
        "message": f"Successfully added author {received_author.name}",
    }
...
def manually_add_author():
    """
    Enables the user to manually add an author
    :return:
    """
    # 1st REQUEST, a GET request
    if request.method == "GET":
...
def update_author():
    """
    Enables the user to update an author, following the same principles of "update_book"
    :return:
    """
    try:
...
def updated_author():
    """
    Enables the user to store the updates made to an author, following the same principles of "updated_book"
    :return:
    """
    try:
    ...
    if len(updated_cover_image) != 0:
        author_to_update.cover_img = df.compile_img_url(
            updated_cover_image, False)
    db.session.commit()
    outcome = {
        "result": 200,
        "message": f"Update of author: id-{author_id} successful"}
...
def delete_author():
    """
    Enables the user to delete an author, following the same principles of "delete_book" with the
    difference that, if an author is deleted, all his books are also deleted.
    """
    try:
        received_author_id = int(request.args.get("author_id", "-1"))
    except ValueError as e:
    ...
    outcome = {
        "result": 200,
        "message": f"Deleting of author: {author_to_delete.name}\n",
    }
    ...
    outcome["message"] = (
        outcome["message"] + f"Deleted author: {author_to_delete.name}\n"
    )
    db.session.commit()
    outcome["message"] = outcome["message"] + \
        f"Deleted author: {author_to_delete.name}"
...
def search():
    """
    Searches the DB within both the Book titles and the author name for a match with the received
    ...
    if search_query == "":
        outcome = {
            "message": f'Empty Search query: "{search_query}"',
            "result": 200}
