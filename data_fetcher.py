...
class BookFetchException(Exception):
    """
    Any exception raised when fetching  author or book information fails
    """


BASE_BOOK_INFO_URL = "https://openlibrary.org/search.json"
FIELDS = (
    "fields=key,title,author_name,editions,editions.key,editions.title,"
    "editions.ebook_access,editions.language,isbn"
)
...
def compile_img_url(olid: str, img_for_book: bool = True) -> str:
    ...
    exists.
    :param olid: The olid for the book or author
    :param img_for_book: An indicator if the image is for a book or an author
    :return:
    """
    ...
    try:
        ...
        print(f"Error: no timely answer received from Open Library API: {e}")
        raise BookFetchException(
            "The request to check cover image for author timed out"
        )
...
def fetch_author_info(user_provided_author: str) -> list:
    ...
    try:
        ...
        print(f"Error: no timely answer received from API {e}")
        raise BookFetchException(
            "The request to get author information timed out")
...
def fetch_year_from_input(date_string: str) -> str:
    """
    In specific situations, the open library returns a data according to format dd mm yyyy. We
    only need the year
    :param date_string:
    :return:
    """
...
def fetch_new_author(author_olid: str) -> Author:
    """
    creates a new Author object based upon the received author_olid (open library id)
    With this call, extra details can be found for a specific author. I.e. sufficient
    information to create a new object.
    :param author_olid:
    :return:
    """
    ...
    new_author = Author(
        name=author_details.get(
            "name",
            ""),
        cover_img=compile_img_url(
            author_olid,
            False),
        birth_year=fetch_year_from_input(
            author_details.get(
                "birth_year",
                "- - -")),
        death_year=fetch_year_from_input(
            author_details.get(
                "death_year",
                "- - -")),
...
def fetch_book_info(user_provided_book: str,
                    user_provided_author_name: str) -> list:
    """
    This function makes a general search call for a book based upon user input. The output is a list of tuples
    (Book object, author name). The background for this combination is that a book object does not have the
    author name which is needed for the user to make a final selection
    ...
    try:
        ...
        print(f"Error: no timely answer received from API: {e}")
        raise BookFetchException(
            "The request to get book information timed out")
    ...
    if book_details.get("numFound", 0) > 0:
        for book_found in book_details["docs"]:
            logging.info(
                f"Result of GET response with URL:\n{request_URL1}:\n{book_found}")
            ...
            if len(found_book):
                ...
                author_name = book_found.get("author_name", [""])[0].strip()[
                    :MAX_AUTHOR
                ]
                candidate_books.append((new_book, author_name))
        print(
            f"We encountered this/these book(s) from open library from author{author_names}",
            candidate_books,
        )
...
def fetch_new_book(selected_book: dict) -> Book:
    """
    Analogue to the "fetch_new_author" only here, the input is already available
    :param selected_book:
    :return:
    """
    book_to_store = Book(
